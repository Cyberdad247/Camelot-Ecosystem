...
- [x] **Milestone 2: Dynamic Knight Swapping & Intent Switchboard** `[backend]`
  - [x] Update `omnivoice-router.ts` WebSocket message protocol to accept a target `knight_id` `[parallel]`
  - [x] Implement keyword extraction in `audio_session.py` to route context dynamically to specific Knights
  - [x] Refactor Kokoro chunk generation in `kitten_service.py` to dynamically swap voice models `[parallel]`
