...
def log_to_ledger(notes: str):
    ...
    try:
        ...
        lines = [line_str for line_str in content.splitlines() if line_str.startswith("|") and not line_str.startswith("| ID")]
        if lines:
            ids = []
            for line in lines:
                parts = line.split("|")
    ...
    for t in targets:
        if t.exists():
            try:
                ...
                cleaned_lines = [line_str for line_str in lines if not line_str.strip().startswith("- [")]
                t.write_text("\n".join(cleaned_lines) + "\n", encoding="utf-8")
