...
function combineFrames(
  frames: Int16Array[],
  maxSamples: number,
): { samples: Int16Array; truncated: boolean } {
...
export class VoiceFirstRuntime {
  ...
  async start(): Promise<void> {
    ...
    const lease = microphoneArbiter.acquire(
      this.options.holderId,
      this.options.reason ?? 'voice capture',
    );
    if (!lease.ok) {
    ...
    try {
      ...
      const createContext =
        this.options.createAudioContext ?? (() => new AudioContext({ latencyHint: 'interactive' }));
      this.context = createContext();
      await this.context.audioWorklet.addModule(
        this.options.workletUrl ?? '/voice-capture.worklet.js',
      );
      ...
      this.node.port.onmessage = (event: MessageEvent<WorkletFrameMessage>) =>
  ...
  private handleWorkletMessage(message: WorkletFrameMessage): void {
    if (
      ...
      message.sampleCount > MAX_FRAME_SAMPLES
    )
      return;
    ...
    const discontinuity =
      message.discontinuity || dropped > this.lastDropped || samples.length !== message.sampleCount;
  ...
  private updateUtterance(frame: VoiceFrame): void {
    ...
    if (this.speechStartedAt !== null && (silence >= silenceMs || elapsed >= maxUtteranceMs)) {
      ...
      const combined = combineFrames(
        this.utteranceFrames,
        Math.round((maxUtteranceMs / 1000) * TARGET_SAMPLE_RATE),
      );
