...
const KNIGHTS_META = [
  {
    name: 'Merlin',
    aliases: ['MERLIN', 'ARCHITECT'],
    pos: { bottom: '20%', left: '45%' },
    sprite: '/assets/knights/merlin.png',
    color: 'shadow-yellow-500/50',
  },
  {
    name: 'Zenith',
    aliases: ['ZENITH', 'SENTINEL'],
    pos: { bottom: '15%', left: '70%' },
    sprite: '/assets/knights/zenith.png',
    color: 'shadow-red-500/50',
  },
  {
    name: 'Anya',
    aliases: ['ANYA', 'COMPILER', 'ANYA_APEE'],
    pos: { bottom: '15%', left: '20%' },
    sprite: '/assets/knights/anya.png',
    color: 'shadow-green-500/50',
  },
  // Percival uses Merlin's sprite for now or a placeholder if missing
  {
    name: 'Percival',
    aliases: ['PERCIVAL', 'GRAPH_WEAVER', 'SIR_PERCIVAL'],
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {KNIGHTS_META.map((knight, idx) => {
        ...
        const stateMatch = knightState?.name
          ? knight.aliases.some((a) => knightState.name.toUpperCase().includes(a))
          : false;
        ...
        const thoughtText = stateMatch
          ? knightState?.last_thought
          : isActive && lastLog.includes('[THOUGHT]')
            ? lastLog.replace('[THOUGHT]', '')
            : null;
        ...
        return (
          ...
          >
            ...
            {/* Sprite Image */}
            <div
              className={`relative w-48 h-64 transition-all duration-500 ${isActive ? 'scale-110 z-20' : 'scale-100 z-10'} ${knight.isGhost ? 'opacity-80 mix-blend-screen' : ''}`}
            >
              <img
                src={knight.sprite}
                alt={knight.name}
            ...
            {/* Label */}
            <div
              className={`mt-2 px-3 py-1 rounded-full border border-white/10 backdrop-blur-md transition-all ${isActive ? 'bg-white/20 text-white' : 'bg-black/40 text-gray-400'}`}
            >
