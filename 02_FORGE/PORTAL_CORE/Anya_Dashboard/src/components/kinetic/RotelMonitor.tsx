    ...
    return (
        <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-4 font-mono shadow-lg relative overflow-hidden">
            <div className="absolute top-0 right-0 p-2 opacity-10">
                <Activity size={48} className={`text-emerald-500 ${status === "RADIANT" ? "animate-pulse" : ""}`} />
            </div>
            ...
            <h3 className="text-emerald-500 text-[10px] font-bold mb-4 flex items-center gap-2 border-b border-emerald-900/30 pb-2 uppercase tracking-widest">
                ...
                <span className={`ml-auto text-[7px] px-1.5 py-0.5 rounded border ${status === "RADIANT" ? "border-emerald-500 text-emerald-500" : "border-red-500 text-red-500"}`}>
                    {status}
                </span>
            </h3>
            ...
            <div className="bg-black/40 p-2 rounded-xl border border-emerald-900/10 h-32 overflow-hidden relative">
                <div className="absolute top-0 right-2 text-[7px] text-emerald-900 uppercase tracking-tighter">LIVE KINETIC FEED</div>
                <div className="space-y-1">
                    {logs.map((log, i) => (
                        <div key={i} className="text-[9px] text-emerald-600/80 truncate font-mono flex gap-2">
                            <span className="text-emerald-900 font-bold shrink-0">[{log.component.toUpperCase()}]</span>
                            <span className={log.level === "ERROR" ? "text-red-500" : log.level === "WARN" ? "text-yellow-500" : ""}>
                                {log.message}
                            </span>
                        </div>
                    ))}
