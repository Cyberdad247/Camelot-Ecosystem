...
function extractRows(content: string, query: string) {
  ...
  return content
    ...
    .filter(({ line }) => {
      ...
      if (!normalized) {
        return line.startsWith('|') || line.startsWith('## ') || line.includes('[CORE]') || line.includes('LOCAL') || line.includes('CLOUD');
      }
...
function extractFileTree(content: string) {
  ...
  return source
    .map((raw, offset): TreeEntry | null => {
      ...
      const looksLikeFolder = clean.includes('/') || clean.includes('[CORE]') || clean.endsWith(':');
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-[1.5rem] border border-fuchsia-400/25 bg-black shadow-2xl shadow-fuchsia-950/30">
      <div className="flex shrink-0 items-center justify-between border-b border-fuchsia-400/20 bg-white/[0.04] px-4 py-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.24em] text-fuchsia-200">Embedded Gradio Sandbox</p>
      ...
      <iframe title="OpenViking Gradio Sandbox" src={GRADIO_URL} className={`${compact ? 'min-h-[300px]' : 'min-h-[520px]'} min-h-0 w-full flex-1 bg-white`} />
    </div>
  );
}
...
export default function OpenVikingDashboard() {
  ...
  const viewDescriptions: Record<ViewMode, string> = {
    bridge: 'Use this when you want the full command bridge: Gradio sandbox beside the live Camelot file map.',
  ...
  const metrics = [
    { label: 'Map Size', value: mapPayload ? formatBytes(mapPayload.bytes) : 'offline', icon: FileText },
  ...
  return (
    <div className="h-full min-h-0 overflow-hidden bg-[#050208] text-slate-100">
      ...
      <main className={`relative mx-auto grid h-full min-h-0 max-w-[1600px] ${cockpitRows} gap-3 ${shellPad}`}>
        <header className="min-h-0 shrink-0 overflow-hidden rounded-[1.5rem] border border-fuchsia-300/25 bg-black/45 shadow-2xl shadow-fuchsia-950/35 backdrop-blur">
          <div className={`${display.compact ? 'px-4 py-3' : 'px-6 py-5'} border-b border-fuchsia-300/20`}>
            <div className="flex flex-wrap items-center justify-between gap-5">
              <div className="flex items-center gap-4">
                <div className={`${display.compact ? 'h-11 w-11' : 'h-16 w-16'} grid place-items-center rounded-2xl bg-fuchsia-500 text-white shadow-lg shadow-fuchsia-500/30`}>
                  <BrainCircuit className={display.compact ? 'h-6 w-6' : 'h-9 w-9'} />
                </div>
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.34em] text-fuchsia-200">Camelot-OS OpenViking</p>
                  <h1 className={`${headerTitle} font-black tracking-tight`}>Throne Room Virtual CPU</h1>
                  <p className={`${display.compact ? 'hidden' : 'block'} mt-2 max-w-5xl text-sm leading-6 text-slate-400`}>
                    Starship command bridge meets Matrix core: Gradio sandbox, Bifrost telemetry, and an interactive live file structure synced from `entiremap.md`.
                  </p>
                </div>
              </div>
              <button
        ...
        <section className="grid min-h-0 shrink-0 gap-3 xl:grid-cols-[0.74fr_1.26fr]">
          ...
          <div className="rounded-[1.5rem] border border-fuchsia-300/20 bg-black/45 p-3 shadow-2xl shadow-fuchsia-950/20 backdrop-blur">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div role="tablist" aria-label="OpenViking display mode" className="flex flex-wrap gap-2">
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 ${
                        active ? 'bg-fuchsia-500 text-white shadow-lg shadow-fuchsia-950/40' : 'bg-white/[0.06] text-slate-400 hover:bg-white/[0.1] hover:text-slate-100'
                      }`}
                    >
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-500">
                ...
                <span className={display.compact ? 'hidden' : ''}>Modified: {formatModified(mapPayload?.modified_ms ?? 0)}</span>
                <span className={isConnected ? 'text-cyan-300' : 'text-amber-300'}>{latestEventText}</span>
                <span className="rounded-full border border-white/10 px-2 py-1 text-slate-400">{display.displayClass}:{display.width}x{display.height}</span>
              </div>
            </div>
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.035] px-4 py-3">
              <p className="text-sm font-semibold leading-6 text-slate-300">{viewDescriptions[viewMode]}</p>
              <button
        ...
        {error && <p className="rounded-2xl border border-red-400/30 bg-red-500/10 p-4 text-sm font-semibold text-red-200">{error}</p>}
        ...
        {showGuide && (
          <section className="mb-3 rounded-[1.5rem] border border-cyan-300/20 bg-cyan-300/10 p-4 shadow-2xl shadow-cyan-950/15">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.24em] text-cyan-200">Operator Guide</p>
                <h2 className="mt-1 text-xl font-black">Start here</h2>
                <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-300">
                  Press <span className="font-black text-fuchsia-100">Sync Entire Map</span> to refresh `entiremap.md`, then use Map to inspect the system tree or Bridge to work with Gradio and the map together.
                </p>
              </div>
              <button
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-3">
              <button onClick={() => setViewMode('bridge')} className="rounded-2xl border border-fuchsia-300/20 bg-black/30 px-4 py-3 text-left transition hover:bg-fuchsia-300/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fuchsia-300">
                <p className="font-black text-fuchsia-100">Bridge Mode</p>
                <p className="mt-1 text-xs leading-5 text-slate-400">Sandbox plus live file map.</p>
              </button>
              <button onClick={() => setViewMode('map')} className="rounded-2xl border border-cyan-300/20 bg-black/30 px-4 py-3 text-left transition hover:bg-cyan-300/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">
                <p className="font-black text-cyan-100">Map Mode</p>
                <p className="mt-1 text-xs leading-5 text-slate-400">Inspect nodes and search Camelot.</p>
              </button>
              <button onClick={syncMap} className="rounded-2xl border border-emerald-300/20 bg-black/30 px-4 py-3 text-left transition hover:bg-emerald-300/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300">
                <p className="font-black text-emerald-100">Refresh Brain Map</p>
                <p className="mt-1 text-xs leading-5 text-slate-400">Pull the latest terminal map.</p>
              </button>
            </div>
          </section>
        )}
        {viewMode === 'bridge' && (
          <section className={`${display.compact ? 'h-auto overflow-visible' : 'h-full overflow-hidden'} grid min-h-0 gap-4 lg:grid-cols-[0.9fr_1.1fr]`}>
        ...
        {viewMode === 'raw' && (
          <section className={`${display.compact ? 'h-auto' : 'h-full min-h-0 overflow-hidden'} flex flex-col rounded-[2rem] border border-fuchsia-300/20 bg-black/60 p-5 shadow-2xl shadow-fuchsia-950/20 backdrop-blur`}>
            <div className="mb-4 flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.26em] text-fuchsia-200">Raw Brain Map</p>
                <h2 className="text-2xl font-black">Terminal Mirror</h2>
              </div>
              <p className="max-w-xl text-right text-xs leading-5 text-slate-500">{mapPayload?.path ?? 'C:\\Users\\vizio\\CAMELOT_OS\\entiremap.md'}</p>
            </div>
            <pre className={`${display.compact ? 'max-h-[58dvh]' : 'min-h-0 flex-1'} overflow-auto rounded-2xl border border-fuchsia-300/20 bg-slate-950 p-4 text-xs leading-5 text-cyan-100`}>
...
function VirtualCpu({
  ...
  return (
    <section className={`${compact ? 'min-h-[172px]' : 'min-h-[300px]'} relative overflow-hidden rounded-[1.5rem] border border-fuchsia-300/25 bg-black/55 p-4 shadow-2xl shadow-fuchsia-950/40 backdrop-blur`}>
      <div className="absolute inset-x-8 top-10 h-32 rounded-[50%] border border-fuchsia-400/25 bg-fuchsia-500/10 blur-sm" />
      <div className={`${compact ? 'top-10 h-40 w-28' : 'top-12 h-64 w-40'} absolute left-1/2 -translate-x-1/2 rounded-t-[5rem] bg-gradient-to-b from-slate-950 via-black to-black shadow-[0_0_80px_rgba(217,70,239,0.42)]`} />
      <div className={`${compact ? 'top-12 h-16 w-16' : 'top-16 h-24 w-24'} absolute left-1/2 -translate-x-1/2 rounded-full bg-black shadow-[0_0_35px_rgba(6,182,212,0.28)]`} />
      <div className={`${compact ? 'top-20 h-3 w-20' : 'top-28 h-4 w-32'} absolute left-1/2 -translate-x-1/2 rounded-full border border-cyan-200/50 bg-cyan-200/10`} />
      <div className={`${compact ? 'top-24' : 'top-36'} absolute inset-x-0 h-px bg-fuchsia-400/40 shadow-[0_0_24px_rgba(217,70,239,0.9)]`} />

      <div className="relative z-10 flex h-full flex-col justify-between gap-8">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.3em] text-fuchsia-200">Virtual CPU</p>
            <h2 className="mt-2 text-3xl font-black">Camelot Core</h2>
          </div>
          <div className={`rounded-full px-3 py-2 text-xs font-black uppercase tracking-[0.18em] ${isConnected ? 'bg-cyan-300/15 text-cyan-200' : 'bg-amber-300/15 text-amber-200'}`}>
            {isConnected ? 'online' : 'offline'}
          </div>
        </div>

        <div className={`${compact ? 'hidden' : 'grid'} gap-3 md:grid-cols-3 xl:grid-cols-1 2xl:grid-cols-3`}>
          <div className="rounded-2xl border border-white/10 bg-black/60 p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Mapped Systems</p>
            <p className="mt-2 text-3xl font-black text-cyan-100">{activeSignals}/{signals.length}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-black/60 p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Selected Node</p>
            <p className="mt-2 truncate text-lg font-black text-fuchsia-100">{selectedEntry?.name ?? 'none'}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-black/60 p-4">
        ...
        <div className={`${compact ? 'hidden' : 'block'} rounded-2xl border border-cyan-300/20 bg-cyan-300/10 p-4`}>
          <div className="mb-2 flex items-center gap-2">
            <Cpu className="h-4 w-4 text-cyan-200" />
            <p className="text-xs font-black uppercase tracking-[0.24em] text-cyan-200">Bridge Event</p>
          </div>
...
function MapExplorer({
  ...
  return (
    <section className={`${compact ? 'h-auto overflow-visible' : 'h-full min-h-0 overflow-auto'} grid gap-5 ${expanded ? '2xl:grid-cols-[0.88fr_1.12fr]' : ''}`}>
      <div className="min-h-0 rounded-[2rem] border border-fuchsia-300/20 bg-black/55 p-5 shadow-2xl shadow-fuchsia-950/20 backdrop-blur">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.26em] text-fuchsia-200">Interactive File Structure</p>
        ...
        <div className={`${compact ? 'hidden' : 'mb-4 grid'} gap-3 md:grid-cols-3`}>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Selected Line</p>
        ...
        <div className="grid min-h-0 gap-4 xl:grid-cols-[1fr_0.9fr]">
          <div className={`${compact ? 'max-h-[280px]' : 'max-h-[46dvh]'} overflow-auto rounded-2xl border border-fuchsia-300/20 bg-slate-950 p-3 font-mono text-xs`}>
                ...
                return (
                  <button
                    ...
                    className={`grid w-full grid-cols-[1rem_1.25rem_1fr] items-center gap-2 rounded-lg px-2 py-1.5 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 ${
                      active ? 'bg-fuchsia-500/20 text-fuchsia-100' : 'text-slate-300 hover:bg-white/10'
                    }`}
                    style={{ paddingLeft: `${8 + entry.depth * 12}px` }}
                  >
                    <ChevronRight className={`h-3 w-3 ${active ? 'text-cyan-200 opacity-100' : 'opacity-20'}`} />
                    <Icon className={`h-4 w-4 ${entry.core ? 'text-cyan-200' : entry.kind === 'folder' ? 'text-fuchsia-200' : 'text-slate-500'}`} />
            ...
            {selectedEntry ? (
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 break-words text-lg font-black text-cyan-50">{selectedEntry.name}</p>
                </div>
                ...
                <div>
                  ...
                  <pre className="mt-2 whitespace-pre-wrap rounded-xl bg-black/60 p-3 text-xs leading-5 text-slate-300">{selectedEntry.raw}</pre>
                </div>
              </div>
            ) : (
      ...
      <div className="grid gap-5 lg:grid-cols-2">
          ...
          <div className={`${compact ? 'max-h-[220px]' : 'max-h-[360px]'} space-y-2 overflow-auto`}>
            {headings.length === 0 ? (
              ...
              headings.map((heading) => (
                <div key={heading} className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm font-semibold text-slate-300">
                  {heading}
                </div>
              ))
            )}
        ...
        <div className="rounded-[2rem] border border-white/10 bg-black/45 p-5 backdrop-blur">
          ...
          <div className="grid gap-2">
            {signals.map((signal) => (
              <div key={signal.key} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2">
                <span className="text-sm font-semibold text-slate-300">{signal.label}</span>
                <span className={`flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-[0.16em] ${signal.active ? 'bg-cyan-300/15 text-cyan-200' : 'bg-slate-700 text-slate-400'}`}>
      ...
      <div className="rounded-[2rem] border border-white/10 bg-black/45 p-5 backdrop-blur">
        ...
        <div className={`${compact ? 'max-h-[240px]' : 'max-h-[360px]'} overflow-auto rounded-2xl border border-white/10 bg-slate-950 p-3 font-mono text-xs`}>
          {rows.length === 0 ? (
            <p className="p-3 text-slate-500">No matching map rows yet.</p>
          ) : (
            rows.map((row) => (
              <div key={`${row.index}-${row.line}`} className="grid grid-cols-[4rem_1fr] gap-3 rounded-lg px-2 py-1.5 text-slate-300 hover:bg-white/10">
