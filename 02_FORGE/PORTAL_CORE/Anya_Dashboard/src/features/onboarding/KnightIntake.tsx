...
const KNIGHT_ROSTER = {
  GUARDIAN: {
    name: 'The Guardian',
    voice:
      'I am your silent sentinel. My optics never blink; your search rank is my singular purpose. The local terrain is secured.',
    icon: Shield,
  ...
  HUNTER: {
    name: 'The Hunter',
    voice:
      'I smell the scent of wasted competitor spend. I strike where they are weak and harvest leads with lethal precision. ROI is my only metric.',
    icon: Target,
    color: 'text-rose-400',
  ...
  ARCHITECT: {
    ...
    voice:
      'I weave the threads of your enterprise into a singular, self-healing lattice. I handle the labor; you handle the vision. Your time is reclaimed.',
    icon: Cpu,
    color: 'text-cyan-400',
...
export default function KnightIntake() {
  const [step, setStep] = useState<'anya-intro' | 'tier-scout' | 'knight-deploy' | 'anya-outro'>(
    'anya-intro',
  );
    ...
    <div className="flex flex-col items-center justify-center space-y-6 text-center animate-in fade-in duration-700">
      ...
      <div className="space-y-2">
        ...
        <p className="text-slate-400 max-w-md mx-auto text-lg italic">
          "Anya L7 online! Ready to forge your legacy? We're ditching the boring forms and deploying
          real heat. Let's find your Knight! Word!"
        </p>
      </div>
      <button
  ...
  const renderTierScout = () => (
    <div className="grid gap-6 animate-in slide-in-from-bottom-8 duration-500">
      <div className="text-center space-y-2 mb-4">
        ...
        <p className="text-slate-500 italic text-sm italic">
          // SIR_HERMES: "Terrain mapped. Pick your point of attack, Sovereign."
        </p>
      </div>
            ...
            >
              <div
                className={cn(
                  ...
                  selectedTier === key ? knight.bg : 'bg-white/5',
                )}
              >
                <Icon
                  className={cn('w-6 h-6', selectedTier === key ? knight.color : 'text-slate-500')}
                />
              </div>
              <div>
  ...
  const renderKnightDeploy = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center space-y-8 animate-in zoom-in-95 duration-500">
        <div
          className={cn(
            'p-8 rounded-[2.5rem] border-2 flex items-center justify-center shadow-2xl animate-bounce',
            knight?.border,
            knight?.bg,
          )}
        >
  ...
  const renderAnyaOutro = () => (
    <div className="flex flex-col items-center justify-center space-y-6 text-center animate-in fade-in duration-700">
      ...
      <div className="space-y-2">
        ...
        <p className="text-slate-400 max-w-md mx-auto text-lg italic">
          "The intake is optimized, your Knight is hunting, and your life just got a lot easier.
          System status is RADIANT. Word!"
        </p>
      </div>
      <div className="pt-4">
        <p className="text-xs text-slate-600 uppercase tracking-widest font-black">
          Anya L7 | Camelot Apex v.406
        </p>
