...
export default function LegalDeck({ cartridge, onDispatch, dispatching }: DeckProps) {
  ...
  const submit = () =>
    onDispatch(intent, { analysis_type: analysisType, jurisdiction, min_severity: minSeverity, pii_detection: piiDetect, agent_armor: agentArmor });

  return (
    <div className="space-y-5">
      <p className="text-xs text-slate-500">SIR_SENTINEL — Agent-Armor v2.0 + PDG compliance gate.</p>

      <div className="space-y-2">
        <label className="text-xs font-semibold uppercase tracking-widest text-purple-400">Document / Scenario</label>
        <textarea rows={4} placeholder="Paste contract text, describe the compliance scenario, or enter the legal question…"
      ...
      <div>
        <label className="text-xs font-semibold uppercase tracking-widest text-slate-500 mb-2 block">Analysis Type</label>
          ...
          {(['contract', 'compliance', 'risk', 'ip', 'privacy'] as AnalysisType[]).map((t) => (
            <button key={t} onClick={() => setAnalysisType(t)}
              className={cn('w-full text-left rounded-lg border px-3 py-2 transition-colors',
                analysisType === t ? 'bg-purple-950/50 border-purple-500/40' : 'border-slate-700 hover:border-slate-600'
              )}
            >
              <p className={cn('text-xs font-bold capitalize', analysisType === t ? 'text-purple-300' : 'text-slate-400')}>{t}</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          >
            {['US', 'EU', 'UK', 'CA', 'Global'].map((j) => <option key={j}>{j}</option>)}
          </select>
        </div>
        <div>
      ...
      <div className="flex flex-wrap gap-4">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={piiDetect} onChange={(e) => setPiiDetect(e.target.checked)} className="accent-purple-500" />
          <span className="text-xs text-slate-400">PII Detection</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={agentArmor} onChange={(e) => setAgentArmor(e.target.checked)} className="accent-purple-500" />
