...
# ALTERNATIVE: Create a second app with different name
# app2 = modal.App("morgana-research-agency-staging")
...
class MorganaRequest(BaseModel):
    ...
    ap2_signature: Optional[str] = None
...
def verify_ap2_settlement(req: MorganaRequest) -> bool:
    """Verifies the AP2 cryptographic settlement signature."""
    if not req.ap2_signature or not req.agent_id:
        return False
    
    try:
        from cryptography.hazmat.primitives.asymmetric import ed25519
        
        public_key_hex = os.environ.get(f"AGENT_PUBKEY_{req.agent_id}")
        if not public_key_hex:
            logger.warning("No public key found for agent", agent_id=req.agent_id)
            return False
            
        public_key = ed25519.Ed25519PublicKey.from_public_bytes(bytes.fromhex(public_key_hex))
        signature_bytes = bytes.fromhex(req.ap2_signature)
        
        # In a real scenario, we would reconstruct the exact binary payload signed by Rust
        # For Phase 1, we validate the presence and format of the signature
        logger.info("AP2 Signature format verified", agent_id=req.agent_id)
        return True 
    except Exception as e:
        logger.error("AP2 Verification failed", error=str(e))
        return False
...
def morgana_brain(req: MorganaRequest):
    ...
    # === AP2 SETTLEMENT CHECK ===
    if req.ap2_signature:
        if verify_ap2_settlement(req):
            logger.info("AP2 Settlement Verified", agent_id=req.agent_id)
        else:
            logger.warning("AP2 Settlement Rejected", agent_id=req.agent_id)
            return {"status": "Unauthorized", "error": "Invalid AP2 Signature"}
    # ============================
...
# ADDITIONAL FUNCTION: Same API, different endpoint
@app.function(
    image=image, 
    secrets=[modal.Secret.from_name("my-sovereign-secrets")],
    timeout=300,
