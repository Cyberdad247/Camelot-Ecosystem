...
//! Async telemetry collector daemon (merged from kinetic_edge/rotel).
...
fn with_auth() -> impl Filter<Extract = ((),), Error = warp::Rejection> + Clone {
    warp::header::optional::<String>("x-api-key")
        ...
        .and_then(
            |key: Option<String>, auth: Option<String>, query: std::collections::HashMap<String, String>| async move {
...
/// Run the collector daemon: POST /v1/logs, GET /v1/stream (SSE), GET /status.
pub async fn run(port: u16) {
    ...
    println!("ROTEL [KINETIC_GATE] Initialized on :{port}");
    ...
    let logs_route = warp::post()
        ...
        .map(|_, entry: LogEntry, counter: Arc<AtomicUsize>, tx: broadcast::Sender<LogEntry>| {
            ...
            println!("[ROTEL::{count}] {ts} | {} | {}", entry.component, entry.message);
    ...
    let stream_route = warp::get()
        ...
        .map(|_, tx: broadcast::Sender<LogEntry>| {
            let stream = BroadcastStream::new(tx.subscribe()).map(|msg| match msg {
...
#[allow(dead_code)]
fn _infallible_marker() -> Option<Infallible> {
    None
}
