...
class ToolRegistry:
    def __init__(self, *, with_builtins: bool = False):
        ...
        if with_builtins:
            ...
            self.register("sys.read_file", _sys_read_file)
            self.register("sys.write_file", _sys_write_file)
            self.register("sys.advisor", _sys_advisor)
...
def _sys_read_file(params: Dict[str, Any]) -> Dict[str, Any]:
...
def _sys_write_file(params: Dict[str, Any]) -> Dict[str, Any]:
...
def _sys_advisor(params: Dict[str, Any]) -> Dict[str, Any]:
    """PAUSE EXECUTION. Call MERLIN_Ω for strategic pivot when blocked."""
    error_context = str(params.get("error_context", ""))
    attempted_fix = str(params.get("attempted_fix", ""))
    return {
        "status": "PAUSED",
        "advisory_request": {
        ...
        "message": "Cognitive interrupt triggered. Awaiting MERLIN_Ω advisor pivot.",
    }
