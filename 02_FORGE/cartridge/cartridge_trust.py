...
# =========================================
# PHASE 2 — PUBLISHER REGISTRY
# =========================================

# A Publisher is a stable identity (e.g. "acme-cartridge-works", "camelot-core",
# "lisa-invisioned") that owns a rotating set of kids. The same kid must never
# appear under two different publishers (collision breaks the SOC2 chain), and
# every cartridge must be signed by a kid that the named publisher currently owns.
#
# Storage: ~/.camelot/publishers.json or env CAMELOT_CARTRIDGE_PUBLISHERS.
# Wire format: { "publishers": { "<publisher_id>": { "kids": ["<kid>", ...], "active": true, "added_at": "..." } } }

PUBLISHERS_PATH_DEFAULT = KEY_DIR / "publishers.json"


class PublisherEntry:
    """A publisher identity + the set of kids it currently owns.

    Phase 3: also carries the set of public keys (one per kid) needed
    for Ed25519 verification in the browser. The mapping is kid -> base64
    public key (32 bytes raw, base64-encoded, matching the
    ``TrustStore.add_key(public_key_b64=...)`` convention).
    """

    __slots__ = ("publisher_id", "kids", "public_keys", "active", "added_at", "note")

    def __init__(self, publisher_id: str, kids: list[str], active: bool = True,
                 added_at: Optional[str] = None, note: str = "",
                 public_keys: Optional[dict[str, str]] = None):
        self.publisher_id = publisher_id
        self.kids = list(kids)
        self.public_keys = dict(public_keys) if public_keys else {}
        self.active = active
        self.added_at = added_at or _now().isoformat()
        self.note = note
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "PublisherEntry":
        return cls(
            publisher_id=data["publisher_id"],
            kids=list(data.get("kids", [])),
            active=bool(data.get("active", True)),
            added_at=data.get("added_at"),
            note=data.get("note", ""),
            public_keys=dict(data.get("public_keys", {})),
        )

    def owns(self, kid: str) -> bool:
        return self.active and kid in self.kids


class PublisherRegistry:
    """
    Maps ``publisher_id`` -> ``PublisherEntry``. The verification path is:

        manifest.publisher_id  →  resolve_publisher(publisher_id)  →  owns(kid)?

    A cartridge is trusted iff:
      1. The publisher_id is in the registry AND ``active``.
      2. The signature's kid is in that publisher's ``kids`` list.
      3. The key's status in the TrustStore is usable_for_verify (active or rotated).
      4. The cartridge is not in the RevocationList.
      5. The cryptographic verification (cartridge_crypto.verify) succeeds.
    """

    def __init__(self, path: Optional[str | Path] = None):
        self.path = Path(
            path
            or os.getenv("CAMELOT_CARTRIDGE_PUBLISHERS")
            or PUBLISHERS_PATH_DEFAULT
        )
        self.publishers: dict[str, PublisherEntry] = {}
        self._load()
    ...
    def _load(self) -> None:
        if self.path.exists():
            try:
                raw = json.loads(self.path.read_text(encoding="utf-8"))
                self.publishers = {
                    pid: PublisherEntry.from_dict(p)
                    for pid, p in raw.get("publishers", {}).items()
                }
    ...
    def save(self) -> None:
        ...
        self.path.write_text(
            json.dumps(
                {"publishers": {p: e.to_dict() for p, e in self.publishers.items()}},
                indent=2,
            ),
            encoding="utf-8",
        )

    def add_publisher(self, publisher_id: str, kids: list[str], *,
                      active: bool = True, note: str = "") -> PublisherEntry:
        if publisher_id in self.publishers:
            raise ValueError(f"publisher '{publisher_id}' already registered")
        entry = PublisherEntry(publisher_id=publisher_id, kids=kids,
                                active=active, note=note)
        self.publishers[publisher_id] = entry
        self.save()
        return entry

    def add_kid_to_publisher(self, publisher_id: str, kid: str) -> None:
        if publisher_id not in self.publishers:
            raise KeyError(f"unknown publisher: {publisher_id}")
        if kid not in self.publishers[publisher_id].kids:
            self.publishers[publisher_id].kids.append(kid)
            self.save()

    def add_public_key_to_publisher(
        self, publisher_id: str, kid: str, public_key_b64: str
    ) -> None:
        """Register an Ed25519 public key (base64) for a kid under a publisher.

        The kid must already be owned by the publisher (call
        ``add_kid_to_publisher`` first). The public key is stored as-is
        (no decoding) and emitted verbatim by ``to_bundle_dict`` so the
        browser can read it with ``atob()`` during Ed25519 verification.
        """
        if publisher_id not in self.publishers:
            raise KeyError(f"unknown publisher: {publisher_id}")
        entry = self.publishers[publisher_id]
        if kid not in entry.kids:
            raise ValueError(
                f"kid '{kid}' is not owned by publisher '{publisher_id}'; "
                f"call add_kid_to_publisher first"
            )
        entry.public_keys[kid] = public_key_b64
        self.save()

    def resolve(self, publisher_id: str) -> Optional[PublisherEntry]:
        return self.publishers.get(publisher_id)

    def is_kid_owned_by(self, publisher_id: str, kid: str) -> bool:
        entry = self.publishers.get(publisher_id)
        return bool(entry and entry.owns(kid))

    def to_bundle_dict(self) -> dict[str, Any]:
        """Export the registry in the browser bundle format.
        ...
        The shape is::

            {
                ...
                "publishers": [
                    {
                        "publisherId": "acme-cartridge-works",
                        "trustedKids": ["kid-2025", "kid-2026"],
                        "publicKeys": {"kid-2025": "base64...", ...},
                        "active": true,
                        "note": "..."
                    },
                    ...
        ...
        Keys are camelCase to match the TypeScript ``PublisherInfo`` type
        in ``src/lib/v2/cartridge-platform.ts`` so the browser can map
        the bundle directly to ``registerTrustedPublisher`` calls.

        The deterministic ``legacy-v1`` seed is unconditionally filtered
        out (it is bootstrapped lazily on first ``getTrustedPublisher()``
        call in the browser, and ``loadTrustedPublishers`` refuses to
        overwrite existing publishers). There is no opt-in to include
        the seed; if a test needs it, construct the bundle object directly.
        """
        publishers: list[dict[str, Any]] = [
            {
                "publisherId": e.publisher_id,
                "trustedKids": list(e.kids),
                "publicKeys": dict(e.public_keys),
                "active": e.active,
                "note": e.note,
            }
            for e in self.publishers.values()
            if e.publisher_id != "legacy-v1"
        ]
        return {
            "version": "1",
            # _now() returns datetime.now(timezone.utc) so .isoformat()
            # produces an RFC 3339 string with explicit +00:00 offset.
            "exported_at": _now().isoformat(),
            "publishers": publishers,
        }


# ── TrustManager extension for V2 ────────────────────────────────────────────
def _trust_manager_verify_v2(manager: "TrustManager", manifest: Any) -> tuple[bool, str]:
    """
    V2 verification path: publisher check + the existing V1 crypto checks.

    The V2 manifest carries a ``publisher_id``; the publisher must be
    registered and must own the kid the signature was made with. Then the
    existing TrustManager.verify path runs (status/window/revocation/crypto/audit).
    """
    # V1 legacy shim (sha256 == V1_LEGACY_SHA256): mirror the browser
    # isV1Legacy() short-circuit and delegate to the V1 verify path
    # (signature + status/window/revocation). V1 cartridges do not
    # carry a publisher_id that resolves in the registry; the legacy-v1
    # publisher is bootstrapped on the browser side but is NOT a
    # registry entry on the Python side because V1 cartridges are
    # trusted by the V1 trusted loader, not by the publisher registry.
    from .cartridge_schemas import V1_LEGACY_SHA256
    sha256 = _manifest_field(manifest, "sha256", "")
    if sha256 == V1_LEGACY_SHA256:
        return manager.verify(manifest, _manifest_field(manifest, "signature"))
    if not sha256:
        return False, "manifest sha256 is empty; a V2 manifest must be packed (cartridge_archive.pack) before trust verification"

    publisher_id = _manifest_field(manifest, "publisher_id", "legacy-v1")
    kid = cc.key_id(_manifest_field(manifest, "signature"))
    if kid is None:
        return False, "manifest is unsigned or uses a legacy checksum"

    # Publisher gate
    registry = getattr(manager, "publisher_registry", None)
    if registry is None:
        return False, "no PublisherRegistry configured on the TrustManager"
    entry = registry.resolve(publisher_id)
    if entry is None:
        return False, f"unknown publisher '{publisher_id}'"
    if not entry.active:
        return False, f"publisher '{publisher_id}' is deactivated"
    if not entry.owns(kid):
        return False, (
            f"publisher '{publisher_id}' does not own kid '{kid}' "
            f"(known kids: {entry.kids!r})"
        )

    # Delegate to the V1 path (which validates status/window/revocation/crypto).
    return manager.verify(manifest, _manifest_field(manifest, "signature"))


def install_publisher_registry(manager: "TrustManager", registry: PublisherRegistry) -> None:
    """Attach a PublisherRegistry to a TrustManager. The V2 verify path uses it."""
    manager.publisher_registry = registry  # type: ignore[attr-defined]
    manager.verify_v2 = lambda m: _trust_manager_verify_v2(manager, m)  # type: ignore[attr-defined]
