...
### 1. FORMICA (Ants) [Δ]
- **Role:** Parallel File Rewriting & Refactoring.
- **Action:** Rewrite 15-50 files simultaneously via `parallel`.
- **Constraint:** Context limited to 150 tokens.

### 2. PONGID (Gorillas) [🦍]
- **Role:** API heavy-lifting and SDK Integration.
- **Action:** Pure tool-use for Shopify, Modal, or GitHub.
- **Reliability:** Zero hallucination goal.

### 3. CASTOR (Beavers) [▩]
- **Role:** Infrastructure & Isolation Dams.
- **Action:** Spawning gVisor sandboxes and Docker containers.
- **Safety:** Hard isolation from host OS.

### 4. ARACHNE (Spiders) [🕷️]
- **Role:** Headless Browsing & Scraping.
- **Action:** Navigating DOM and verifying UI flows via Playwright.

## Operational Flow (SARDA)
