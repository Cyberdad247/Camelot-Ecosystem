...
| 5 | `fixture-gate` | `python -m pytest tests/test_harness_fixtures.py` | The 29 `harness/fixtures/*/README.md` specs: every README is well-formed (title, scenario, `Verify:` clause, `§x.y` ref); the confirmed/planned/aspirational evidence manifest is complete; and the confirmed fixtures actually run against live code (receipt chain §11.3 + operator-request gate §12.2/§13.1/§19.2). Aspirational fixtures stay explicitly untested — the gate reports the gap instead of claiming coverage. |
...
| 2026-08-24 | `bash harness/gate.sh` (defaults) | ✅ PASS | — | Added check 5 `fixture-gate` (29 fixture READMEs + evidence manifest + confirmed wiring). All five checks green. |
