...
```mermaid
graph LR
    Action[Kinetic Strike] --> Telemetry[Sir Watcher: Metrics]
    Telemetry --> Grader[Sir Grader: Analysis]
    Grader --> Score{Grade A/B/C/F}
    
    Score -->|Success| XP[Grant XP & Level Up]
    Score -->|Failure| Audit[Sentinel Review & Penalty]
    
    XP --> Evolution[Unlock Omega Status / New Skills]
    Evolution --> Action
```
