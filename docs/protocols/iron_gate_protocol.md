...
```mermaid
sequenceDiagram
    participant K as The Swarm (L5/L2)
    participant G as Iron Gate (L6)
    participant S as Sovereign (User)
    
    K->>G: Proposes Kinetic Strike (Edit >10 lines / Delete / Kernel)
    G->>G: Log Hash to PROVENANCE_LEDGER
    G->>S: Request Authorization: [👤✅]
    Note over S: Sovereign reviews the Logic & Diff
    S->>G: Authorizes via keyword or Button
    G->>K: Unlocks Execution Path
    K->>K: Kinetic Strike Executed
```
