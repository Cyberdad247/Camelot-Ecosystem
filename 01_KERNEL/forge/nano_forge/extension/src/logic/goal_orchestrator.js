    ...
    while (!this.isFinished()) {
      ...
      for (const goal of readyGoals) {
        ...
        try {
          ...
          const evaluation = await MissionEvaluator.evaluate(
            goal.description,
            transcript,
            finalUrl,
          );
          ...
          if (goal.status === 'COMPLETED') {
            console.log(
              `[ORCHESTRATOR] Goal ${goal.id} finished. Pondering future requirements...`,
            );
            ...
            if (predictions && Array.isArray(predictions)) {
              predictions.forEach((p) => {
                if (p.confidence > 0.7) {
                  console.log(
                    `[ORCHESTRATOR] (PREDICT) High-confidence move detected: "${p.prediction}" (${p.rationale})`,
                  );
                }
              });
            }
