    ...
    static async perform(tabId, intent, laneId = 'default_lane') {
        ...
        if (typeof process_via_offscreen !== 'undefined') {
            ...
            if (reflection && reflection.approved === false) {
                ...
                await transcript.log('VETO', { reason: reflection.critique, suggestion: reflection.suggestion });
            ...
            try {
                ...
                if (attempt > MAX_RETRIES) {
                    ...
                    if (diagnosis.healed) {
                        ...
                        const healedIntent = {
                            ...intent,
                            selectorOverride: diagnosis.strategy === 'SELECTOR' ? diagnosis.fallback_selector : null,
                            coordinateOverride: diagnosis.strategy === 'COORDINATE' ? { x: diagnosis.x, y: diagnosis.y } : null
                        };
        ...
        const result = await chrome.scripting.executeScript({
            ...
            func: (intent) => {
                if (intent.selectorOverride) {
                    ...
                    if (el) return { found: true, id: el.getAttribute('data-nano-id') || 'healed', element: { tagName: el.tagName } };
                }
        ...
        if (resolution && resolution.found) {
            ...
            await chrome.scripting.executeScript({
                ...
                func: (id, action, value, selectorOverride, coordinateOverride) => {
                    ...
                    const el = (selectorOverride ? document.querySelector(selectorOverride) : null) ||
                ...
                args: [resolution.id, intent.action, intent.value, intent.selectorOverride, intent.coordinateOverride]
            });
