...
export class LLMClient {
  ...
  async generate(
    ...
    console.log(
      `[LLM] Generating... Mode: ${complexity}, Hybrid: ${this.config.hybridMode}, Format: ${format}, Images: ${images.length}`,
    );
    ...
    if (judgement.environment === 'LOCAL') {
      ...
      // CLOUD REQUIRED
      if (this.config.allowCloudOffload || images.length > 0) {
        // Vision implies consent? Or should we check? allowCloud is strictly for 'offloading', maybe Vision is a separate permission?
  ...
  async extract(rawText, schema, userGoal = 'Extract all matching entities') {
    ...
    if (rawText.length > CHUNK_THRESHOLD) {
      console.log(
        `[HYDRA] Input size ${rawText.length} > ${CHUNK_THRESHOLD}. Engaging Chunk Manager.`,
      );
...
`;
    ...
    try {
      ...
      const cleanParams = responseText
        .replace(/```json/g, '')
        .replace(/```/g, '')
        .trim();
...
const llmInstance = new LLMClient();
if (typeof window !== 'undefined') {
  window.LLM = llmInstance;
}
