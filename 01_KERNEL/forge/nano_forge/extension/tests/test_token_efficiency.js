...
const nestedReduction = ((estimateTokens(nestedHTML) - estimateTokens(nestedTOON)) / estimateTokens(nestedHTML)) * 100;
assert(nestedReduction > 30, "Test 6: Nested structure >30% reduction");

// Test 7: Attribute stripping effectiveness
const htmlWithAttrs = '<button id="btn-123" class="btn btn-primary btn-lg" data-toggle="modal" aria-label="Close">Click</button>';
const toonWithoutAttrs = '[1] button "Click"';

const attrReduction = ((estimateTokens(htmlWithAttrs) - estimateTokens(toonWithoutAttrs)) / estimateTokens(htmlWithAttrs)) * 100;
...
const scriptRemoval = ((estimateTokens(htmlWithScripts) - estimateTokens(toonWithoutScripts)) / estimateTokens(htmlWithScripts)) * 100;
...
const whitespaceReduction = ((estimateTokens(htmlWithWhitespace) - estimateTokens(toonCompressed)) / estimateTokens(htmlWithWhitespace)) * 100;
