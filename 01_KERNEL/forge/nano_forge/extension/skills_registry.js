...
const SKILL_LIBRARY = {
  ...
  LOCATE_ELEMENT_VISUALLY: async (data) => {
    ...
    const raw = await window.LLM.generate(
      prompt,
      'You are Omega_OCULAR, a precision visual navigator. Return ONLY JSON.',
      'HIGH',
      'JSON',
      images,
    );
  ...
  SYNTHESIZE_SKILL: async (data) => {
    ...
    const raw = await window.LLM.generate(
      prompt,
      'You are Omega_FORGE, a master of just-in-time skill synthesis. Output ONLY JSON.',
    );
    return parseJson(raw);
  },
  ...
  REFLECT_ON_ACTION: async (data) => {
    ...
    const raw = await window.LLM.generate(
      prompt,
      "You are Omega_MIRROR, a meta-cognitive auditor. Analyze the logic behind the swarm's next move. Output ONLY JSON.",
    );
    return parseJson(raw);
  },
  ...
  PICK_SEMANTIC_ANCHOR: async (data) => {
    ...
    const raw = await window.LLM.generate(
      prompt,
      'You are Omega_ANCHOR, a spatial-semantic navigator. Return ONLY JSON.',
    );
    return parseJson(raw);
  },
  ...
  DECOMPOSE_MISSION: async (data) => {
    ...
    const raw = await window.LLM.generate(
      prompt,
      'You are Omega_STRATEGIST, a master of recursive mission decomposition. Output ONLY JSON.',
    );
    return parseJson(raw);
  },
  ...
  PREDICT_NEXT_MOVE: async (data) => {
    ...
    const raw = await window.LLM.generate(
      prompt,
      'You are Omega_PREDICT, a predictive intelligence engine. Output ONLY JSON.',
    );
    return parseJson(raw);
  },
  ...
  EXTRACT_PATTERN: async (data) => {
    ...
    const raw = await window.LLM.generate(
      prompt,
      'You are Omega_HIVE, a collective intelligence system. Output ONLY JSON.',
    );
    return parseJson(raw);
  },
};
