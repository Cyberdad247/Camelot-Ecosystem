        ...
        async with self._lock or self.nullcontext():
            try:
                ...
                if cached_result:
                    ...
                    extracted_content = (
                        None if not extracted_content or extracted_content == "[]" else extracted_content
                    )
                ...
                if config and config.proxy_rotation_strategy:
                    ...
                    if next_proxy:
                        self.logger.info(
                            message="Switch proxy: {proxy}", tag="PROXY", params={"proxy": next_proxy.server}
                        )
                ...
                return CrawlResultContainer(CrawlResult(url=url, html="", success=False, error_message=error_message))
    ...
    async def aprocess_html(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ValueError(f"Process HTML, Failed to extract content from the website: {url}, error: {str(e)}") from e
        ...
        self.logger.url_status(
            url=_url, success=True, timing=int((time.perf_counter() - t1) * 1000) / 1000, tag="SCRAPE"
        )
        # self.logger.info(
