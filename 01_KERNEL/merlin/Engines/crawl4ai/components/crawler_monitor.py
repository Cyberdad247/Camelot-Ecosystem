...
class TerminalUI:
    ...
    def _create_pipeline_panel(self) -> Panel:
        ...
        table.add_row("", "", "", "Requeue Rate", f"{summary.get('requeue_rate', 0):.1f}%")
    ...
    def _create_task_details_panel(self) -> Panel:
        ...
        # Add summary row
        active_tasks = sum(1 for stats in task_stats.values() if stats["status"] == CrawlStatus.IN_PROGRESS.name)
    ...
    def _create_footer(self) -> Panel:
        ...
        footer_content = Columns(
            [Align.left(left_text), Align.center(center_text), Align.right(right_text)], expand=True
        )

        # Create a more visible footer panel
        return Panel(footer_content, border_style="white", padding=(0, 1))  # Add padding for better visibility
...
class CrawlerMonitor:
    ...
    def __init__(self, urls_total: int = 0, refresh_rate: float = 1.0, enable_ui: bool = True, max_width: int = 120):
        """
        ...
        self.queue_stats = {"total_queued": 0, "highest_wait_time": 0.0, "avg_wait_time": 0.0}
        ...
        self.terminal_ui = TerminalUI(refresh_rate=refresh_rate, max_width=max_width) if enable_ui else None
    ...
    def update_task(
        ...
        with self._lock:
            ...
            if status and status.name != old_status:
                ...
                # Track requeues
                if old_status in [CrawlStatus.COMPLETED.name, CrawlStatus.FAILED.name] and not task_stats.get(
                    "counted_requeue", False
                ):
    ...
    try:
        ...
        for i in range(20):
            ...
            if i >= 4 and i < 6:
                ...
                monitor.update_task(
                    task_id=task_id, status=CrawlStatus.IN_PROGRESS, start_time=start_time, memory_usage=8.2
                )
                monitor.update_task(
                    task_id=task_id, status=CrawlStatus.COMPLETED, end_time=end_time, memory_usage=0, peak_memory=15.7
                )
            ...
            if i >= 6 and i < 7:
                ...
                monitor.update_task(
                    task_id=task_id, status=CrawlStatus.IN_PROGRESS, start_time=start_time, memory_usage=12.3
                )
                monitor.update_task(
                    task_id=task_id,
        ...
        monitor.update_queue_statistics(
            total_queued=16, highest_wait_time=120.5, avg_wait_time=60.2  # 20 - 4 (in progress)
        )
