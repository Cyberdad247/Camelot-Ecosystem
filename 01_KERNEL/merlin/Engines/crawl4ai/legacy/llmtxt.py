...
"""
        ...
        try:
            ...
            for response, file_path in zip(responses, doc_batch, strict=False):
                try:
                    ...
                    index_content = re.sub(r"\n\s*\n", "\n", index_content_match.group(1)).strip()
                    if index_content:
    ...
    def preprocess_text(self, text: str) -> List[str]:
        ...
        for part in parts:
            if "(" in part and ")" in part:
                code_tokens = re.findall(r'[\w_]+(?=\()|[\w_]+(?==[\'"]{1}[\w_]+[\'"]{1})', part)
    ...
    async def generate_index_files(self, force_generate_facts: bool = False, clear_bm25_cache: bool = False) -> None:
        ...
        if not force_generate_facts:
            md_files = [f for f in md_files if not (self.docs_dir / f.name.replace(".md", ".q.md")).exists()]
    ...
    def generate(self, sections: List[str], mode: str = "extended") -> str:
        ...
        all_files = glob.glob(str(self.docs_dir / "[0-9]*.md")) + glob.glob(str(self.docs_dir / "[0-9]*.xs.md"))

        # Extract base names without extensions
        base_docs = {Path(f).name.split(".")[0] for f in all_files if not Path(f).name.endswith(".q.md")}

        # Filter by sections if provided
        if sections:
            base_docs = {doc for doc in base_docs if any(section.lower() in doc.lower() for section in sections)}
    ...
    def search(self, query: str, top_k: int = 5) -> str:
        ...
        ranked_files = sorted(
            file_data.items(),
            key=lambda x: (x[1]["code_match_score"] * 2.0 + x[1]["match_count"] * 1.5 + x[1]["total_score"]),
            reverse=True,
        )[:top_k]
    ...
    def _aggregate_search_scores(
        ...
        for idx, score in enumerate(doc_scores):
            ...
            file_data[file_path]["code_match_score"] = max(file_data[file_path]["code_match_score"], code_match_score)
