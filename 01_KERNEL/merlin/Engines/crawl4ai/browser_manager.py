...
class ManagedBrowser:
    ...
    async def start(self) -> str:
        ...
        try:
            ...
            await self.cleanup()
            raise Exception(f"Failed to start browser: {e}")  # noqa
...
class BrowserManager:
    ...
    def _build_browser_args(self) -> dict:
        ...
        if self.config.accept_downloads:
            browser_args["downloads_path"] = self.config.downloads_path or os.path.join(os.getcwd(), "downloads")
    ...
    async def setup_context(
            ...
            [
                {
                    ...
                    "value": "true",
                    "url": (
                        crawlerRunConfig.url if crawlerRunConfig and crawlerRunConfig.url else "https://crawl4ai.com/"
                    ),
                }
            ]
        )
        ...
        if crawlerRunConfig:
            if crawlerRunConfig.override_navigator or crawlerRunConfig.simulate_user or crawlerRunConfig.magic:
    ...
    def _cleanup_expired_sessions(self):
        ...
        expired_sessions = [
            sid for sid, (_, _, last_used) in self.sessions.items() if current_time - last_used > self.session_ttl
        ]
    ...
    async def close(self):
        ...
        for ctx in self.contexts_by_config.values():
            try:
                ...
                self.logger.error(message="Error closing context: {error}", tag="ERROR", params={"error": str(e)})
