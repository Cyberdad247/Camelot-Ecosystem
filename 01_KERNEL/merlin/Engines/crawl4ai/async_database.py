...
class AsyncDatabaseManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.info(message="Database will be initialized on first use", tag="INIT")
    ...
    async def aget_cached_url(self, url: str) -> Optional[CrawlResult]:
        ...
        async def _get(db):
            async with db.execute("SELECT * FROM crawled_data WHERE url = ?", (url,)) as cursor:
                ...
                try:
                    row_dict["downloaded_files"] = (
                        json.loads(row_dict["downloaded_files"]) if row_dict["downloaded_files"] else []
                    )
                except json.JSONDecodeError:
                    row_dict["downloaded_files"] = []
    ...
    async def acache_url(self, result: CrawlResult):
        ...
        try:
            ...
            self.logger.warning(message=f"Error processing markdown content: {str(e)}", tag="WARNING")
