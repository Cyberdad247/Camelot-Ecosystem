...
class Crawl4aiDockerClient:
    ...
    def __init__(
        ...
        self._http_client = httpx.AsyncClient(
            timeout=timeout, verify=verify_ssl, headers={"Content-Type": "application/json"}
        )
    ...
    async def _check_server(self) -> None:
        ...
        try:
            ...
            raise ConnectionError(f"Cannot connect to server: {str(e)}") from e
    ...
    async def _request(self, method: str, endpoint: str, **kwargs) -> httpx.Response:
        ...
        try:
            ...
            raise ConnectionError(f"Request timed out: {str(e)}") from e
        except httpx.RequestError as e:
            ...
            raise RequestError(f"Server error {e.response.status_code}: {error_msg}") from e
                    ...
                    async for line in response.aiter_lines():
                        if line.strip():
                            ...
                            self.logger.url_status(
                                url=result.get("url", "unknown"), success=True, timing=result.get("timing", 0.0)
                            )
