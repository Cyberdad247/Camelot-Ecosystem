...
class OnlineUAGenerator(UAGen):
    ...
    def _fetch_agents(self):
        try:
            response = requests.get(
                "https://www.useragents.me/", timeout=5, headers={"Accept": "text/html,application/xhtml+xml"}
            )
    ...
    def generate(
        ...
        if browsers:
            filtered_agents = [a for a in filtered_agents if any(b.lower() in a["ua"].lower() for b in browsers)]

        if os:
            os_list = [os] if isinstance(os, str) else os
            filtered_agents = [a for a in filtered_agents if any(o.lower() in a["ua"].lower() for o in os_list)]

        if platforms:
            platform_list = [platforms] if isinstance(platforms, str) else platforms
            filtered_agents = [a for a in filtered_agents if any(p.lower() in a["ua"].lower() for p in platform_list)]
