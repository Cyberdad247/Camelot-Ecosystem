...
class RelevantContentFilter(ABC):
    ...
    def extract_text_chunks(self, body: Tag, min_word_threshold: int = None) -> List[Tuple[str, str]]:
        ...
        def should_break_chunk(tag: Tag) -> bool:
            ...
            return tag.name not in INLINE_TAGS and not (tag.name == "p" and len(current_text) == 0)
    ...
    def is_excluded(self, tag: Tag) -> bool:
        ...
        if tag.name in self.excluded_tags:
            return True
        class_id = " ".join(filter(None, [" ".join(tag.get("class", [])), tag.get("id", "")]))
...
class BM25ContentFilter(RelevantContentFilter):
    ...
    def filter_content(self, html: str, min_word_threshold: int = None) -> List[str]:
        ...
        tokenized_corpus = [
            [self.stemmer.stemWord(word) for word in chunk.lower().split()] for _, chunk, _, _ in candidates
        ]
    ...
    def __init__(
        ...
        if logger:
            ...
            self.logger = AsyncLogger(
                ...
                colors={**AsyncLogger.DEFAULT_COLORS, LogLevel.INFO: LogColor.DIM_MAGENTA},  # Dimmed purple for LLM ops
            )
        else:
            self.logger = None
    ...
    def show_usage(self) -> None:
        ...
        if self.usages:
            ...
            for i, usage in enumerate(self.usages, 1):
                print(
                    f"{i:<10} {usage.completion_tokens:>12,} " f"{usage.prompt_tokens:>12,} {usage.total_tokens:>12,}"
                )