...
def from_serializable_dict(data: Any) -> Any:
    ...
    if isinstance(data, dict) and "type" in data:
        ...
        if hasattr(crawl4ai, data["type"]):
            ...
            if "params" in data:
                # Handle class instances
                constructor_args = {k: from_serializable_dict(v) for k, v in data["params"].items()}
...
class GeolocationConfig:
    def __init__(self, latitude: float, longitude: float, accuracy: Optional[float] = 0.0):
...
class ProxyConfig:
    ...
    def from_string(proxy_str: str) -> "ProxyConfig":
        ...
        if len(parts) == 4:  # ip:port:username:password
            ...
            return ProxyConfig(server=f"http://{ip}:{port}", username=username, password=password, ip=ip)
    ...
    def to_dict(self) -> Dict:
        """Convert to dictionary representation."""
        return {"server": self.server, "username": self.username, "password": self.password, "ip": self.ip}
...
class BrowserConfig:
    ...
    def __init__(
        ...
        if self.user_agent_mode == "random":
            self.user_agent = fa_user_agenr_generator.generate(**(self.user_agent_generator_config or {}))
        else:
            pass
...
class CrawlerRunConfig:
    ...
    _param_defaults = None

    @classmethod
    def _get_param_defaults(cls):
        """Evaluate and cache the parameter defaults for performance."""
        if cls._param_defaults is None:
    ...
    def __init__(
        ...
        # Set all arguments dynamically as properties
        params = locals()
        for key, value in params.items():
            if key not in ("self", "__class__"):
                setattr(self, key, value)

        # Apply fallback defaults for properties that need it
    ...
    def __setattr__(self, name, value):
        ...
        param_defaults = self._get_param_defaults()
        if name in self._UNWANTED_PROPS and value is not param_defaults.get(name):
