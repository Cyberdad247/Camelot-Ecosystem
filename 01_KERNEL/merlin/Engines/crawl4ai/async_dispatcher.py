...
class RateLimiter:
    ...
    def update_delay(self, url: str, status_code: int) -> bool:
        ...
        if status_code in self.rate_limit_codes:
            ...
            state.current_delay = min(state.current_delay * 2 * random.uniform(0.75, 1.25), self.max_delay)
        else:
        ...
        try:
            if self.monitor:
                self.monitor.update_task(
                    task_id, status=CrawlStatus.IN_PROGRESS, start_time=start_time, retry_count=retry_count
                )
            ...
            if self.current_memory_percent >= self.critical_threshold_percent:
                ...
                if self.monitor:
                    self.monitor.update_task(
                        task_id, status=CrawlStatus.QUEUED, error_message="Requeued due to critical memory pressure"
                    )
            ...
            while not self.task_queue.empty() or active_tasks:
                ...
                if not self.memory_pressure_mode and len(active_tasks) < self.max_session_permit:
                    try:
                        ...
                        task = asyncio.create_task(self.crawl_url(url, config, task_id, retry_count))
                        active_tasks.append(task)

                        # Update waiting time in monitor
                        if self.monitor:
                            wait_time = time.time() - enqueue_time
                            self.monitor.update_task(task_id, wait_time=wait_time, status=CrawlStatus.IN_PROGRESS)
    ...
    async def _update_queue_priorities(self):
        ...
        if temp_items and self.monitor:
            ...
            self.monitor.update_queue_statistics(
                total_queued=total_queued, highest_wait_time=highest_wait_time, avg_wait_time=avg_wait_time
            )
    ...
    async def run_urls(
        ...
        try:
            ...
            for url in urls:
                ...
                if self.monitor:
                    self.monitor.add_task(task_id, url)
                task = asyncio.create_task(self.crawl_url(url, config, task_id, semaphore))
                tasks.append(task)
