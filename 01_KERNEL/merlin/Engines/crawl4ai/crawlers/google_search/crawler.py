    ...
    async def run(self, url="", query: str = "", search_type: str = "text", schema_cache_path=None, **kwargs) -> str:
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            config = CrawlerRunConfig(
                ...
                delay_before_return_html=kwargs.get("delay", 2 if search_type == "image" else 1),
            ...
            extracted = {
                key: JsonCssExtractionStrategy(schema=schemas[key]).run(url=url, sections=[result.html])
                for key in schemas
            }
