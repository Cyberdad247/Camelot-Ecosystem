...
def merge_chunks(
    docs: Sequence[str], target_size: int, overlap: int = 0, word_token_ratio: float = 1.0, splitter: Callable = None
) -> List[str]:
...
class RobotsParser:
    ...
    def _get_cached_rules(self, domain: str) -> tuple[str, bool]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("SELECT rules, fetch_time, hash FROM robots_cache WHERE domain = ?", (domain,))
    ...
    def _cache_rules(self, domain: str, content: str):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute("SELECT hash FROM robots_cache WHERE domain = ?", (domain,))
            result = cursor.fetchone()
...
async def get_chromium_path(browser_type) -> str:
    ...
    async with async_playwright() as p:
        ...
        if browser_type.lower() not in browsers:
            raise ValueError(f"Invalid browser type. Must be one of: {', '.join(browsers.keys())}")
...
def get_content_of_website(url, html, word_count_threshold=MIN_WORD_THRESHOLD, css_selector=None, **kwargs):
    ...
    try:
        ...
        for img in body.find_all("img"):
            media["images"].append({"src": img.get("src"), "alt": img.get("alt"), "type": "image"})

        # Extract all video tags into [{src: '', alt: ''}]
        for video in body.find_all("video"):
            media["videos"].append({"src": video.get("src"), "alt": video.get("alt"), "type": "video"})

        # Extract all audio tags into [{src: '', alt: ''}]
        for audio in body.find_all("audio"):
            media["audios"].append({"src": audio.get("src"), "alt": audio.get("alt"), "type": "audio"})
...
def get_content_of_website_optimized(
    ...
    **kwargs,
    ...
    media["images"] = [
        result for result in (process_image(img, url, i, len(imgs)) for i, img in enumerate(imgs)) if result is not None
    ]
...
def process_sections(url: str, sections: list, provider: str, api_token: str, base_url=None) -> list:
    ...
    if provider.startswith("groq/"):
        ...
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(extract_blocks, url, section, provider, api_token, base_url=base_url)
                for section in sections
            ]
...
def normalize_url_for_deep_crawl(href, base_url):
    ...
    normalized = urlunparse(
        (parsed.scheme, netloc, parsed.path.rstrip("/"), parsed.params, query, fragment)  # Normalize trailing slash
    )
...
def normalize_url_tmp(href, base_url):
    ...
    try:
        ...
        raise ValueError(f"Invalid base URL format: {base_url}")  # noqa
