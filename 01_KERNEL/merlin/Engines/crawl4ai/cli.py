        ...
        try:
            ...
            if v.lower() == "true":
                ...
                try:
                    ...
                    raise click.BadParameter(f"Invalid JSON object: {v}")  # noqa
            result[k.strip()] = v
        except ValueError:
            raise click.BadParameter(f"Invalid key=value pair: {pair}")  # noqa
    return result
...
def load_config_file(path: Optional[str]) -> dict:
    ...
    try:
        ...
        raise click.BadParameter(f"Error loading config file {path}: {str(e)}")  # noqa
...
async def run_crawler(url: str, browser_cfg: BrowserConfig, crawler_cfg: CrawlerRunConfig, verbose: bool):
    ...
    async with AsyncWebCrawler(config=browser_cfg) as crawler:
        try:
            ...
            raise click.ClickException(f"Crawling failed: {str(e)}")  # noqa
...
def display_profiles_table(profiles: List[Dict[str, Any]]):
    ...
    for i, profile in enumerate(profiles):
        ...
        # Add row to table
        table.add_row(str(i + 1), profile["name"], profile["path"], created, profile["type"].capitalize(), human_size)
...
async def crawl_with_profile_cli(profile_path, url):
    ...
    output_format = Prompt.ask(
        "[cyan]Output format[/cyan]", choices=["all", "json", "markdown", "md", "title"], default="markdown"
    )
    ...
    while True:
        ...
        for key, value in options.items():
            color = (
                "green"
                if key == "1"
                else "yellow" if key == "2" else "red" if key == "3" else "blue" if key == "4" else "cyan"
            )
...
def browser_start_cmd(browser_type: str, port: int, headless: bool):
    ...
    try:
        ...
        cdp_url = anyio.run(profiler.launch_builtin_browser, browser_type, port, headless)
        ...
        if cdp_url:
            ...
            console.print(
                Panel("[red]Failed to start builtin browser[/red]", title="Builtin Browser Start", border_style="red")
            )
            sys.exit(1)
...
def browser_stop_cmd():
    ...
    try:
        ...
        console.print(
            Panel("[cyan]Stopping builtin browser...[/cyan]", title="Builtin Browser Stop", border_style="cyan")
        )
        ...
        if success:
            ...
            console.print(
                Panel("[red]Failed to stop builtin browser[/red]", title="Builtin Browser Stop", border_style="red")
            )
            sys.exit(1)
    ...
    try:
        ...
        if status["running"]:
            ...
            if not success:
                console.print(
                    Panel(
                        "[red]Failed to stop current browser[/red]", title="Builtin Browser Restart", border_style="red"
                    )
                )
                sys.exit(1)
        ...
        if cdp_url:
            ...
            console.print(
                Panel(
                    "[red]Failed to restart builtin browser[/red]", title="Builtin Browser Restart", border_style="red"
                )
            )
            sys.exit(1)
...
def cdp_cmd(user_data_dir: Optional[str], port: int, browser_type: str, headless: bool, incognito: bool):
    ...
    try:
        ...
        # Run the browser
        cdp_url = anyio.run(profiler.launch_standalone_browser, browser_type, data_dir, port, headless)
...
@click.option(
    "--browser", "-b", type=str, callback=parse_key_values, help="Browser parameters as key1=value1,key2=value2"
)
@click.option(
    "--crawler", "-c", type=str, callback=parse_key_values, help="Crawler parameters as key1=value1,key2=value2"
)
@click.option(
    "--output", "-o", type=click.Choice(["all", "json", "markdown", "md", "markdown-fit", "md-fit"]), default="all"
)
            ...
            if filter_conf["type"] == "bm25":
                crawler_cfg.markdown_generator = DefaultMarkdownGenerator(
                    content_filter=BM25ContentFilter(
                        user_query=filter_conf.get("query"), bm25_threshold=filter_conf.get("threshold", 1.0)
                    )
                )
            elif filter_conf["type"] == "pruning":
                crawler_cfg.markdown_generator = DefaultMarkdownGenerator(
                    content_filter=PruningContentFilter(
                        user_query=filter_conf.get("query"), threshold=filter_conf.get("threshold", 0.48)
                    )
                )
...
Always return valid, properly formatted JSON."""
        ...
        # Handle extraction strategy from config file (only if json-extract wasn't used)
            ...
            if extract_conf["type"] == "llm":
                ...
                crawler_cfg.extraction_strategy = JsonCssExtractionStrategy(schema=schema_data)
            elif extract_conf["type"] == "json-xpath":
                crawler_cfg.extraction_strategy = JsonXPathExtractionStrategy(schema=schema_data)
        ...
        # Run crawler
        result: CrawlResult = anyio.run(run_crawler, url, browser_cfg, crawler_cfg, verbose)
        ...
        raise click.ClickException(str(e))  # noqa
