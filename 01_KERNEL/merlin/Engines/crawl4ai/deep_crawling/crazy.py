...
class BloomFilter:
    ...
    def __contains__(self, item: str) -> bool:
        return all(self.bits[mmh3.hash(item, seed) % self.size] for seed in range(self.hashes))
...
async def main():
    ...
    config: CrawlerRunConfig = CrawlerRunConfig(
        deep_crawl_strategy=strategy, stream=False, verbose=True, cache_mode=CacheMode.BYPASS
    )
