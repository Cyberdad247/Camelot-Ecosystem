...
class ContentTypeFilter(URLFilter):
    ...
    def __init__(
        ...
        self.allowed_types = frozenset(
            t.lower() for t in (allowed_types if isinstance(allowed_types, list) else [allowed_types])
        )
...
class DomainFilter(URLFilter):
    ...
    def __init__(
        ...
        self._allowed_domains = frozenset(self._normalize_domains(allowed_domains)) if allowed_domains else None
        self._blocked_domains = frozenset(self._normalize_domains(blocked_domains)) if blocked_domains else frozenset()
...
class SEOFilter(URLFilter):
    ...
    def __init__(
        ...
        self._kw_patterns = (
            re.compile(r"\b({})\b".format("|".join(map(re.escape, keywords or []))), re.I) if keywords else None
        )
    ...
    async def apply(self, url: str) -> bool:
        ...
        total_score = sum(weight * scores[factor] for factor, weight in self._weights.items())
