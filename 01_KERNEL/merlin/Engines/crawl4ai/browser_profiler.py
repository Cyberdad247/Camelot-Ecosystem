...
class BrowserProfiler:
    ...
    async def create_profile(
        self, profile_name: Optional[str] = None, browser_config: Optional[BrowserConfig] = None
    ) -> Optional[str]:
        """
        ...
        if browser_config is None:
            ...
            browser_config = BrowserConfig(
                browser_type="chromium", headless=False, verbose=True  # Must be visible for user interaction
            )
        else:
            ...
            try:
                ...
                while True:
                    ...
                    if readable:
                        ...
                        if key.lower() == "q":
                            self.logger.info(
                                "Closing browser and saving profile...", tag="PROFILE", base_color=LogColor.GREEN
                            )
                            user_done_event.set()
                            return
    ...
    def list_profiles(self) -> List[Dict[str, Any]]:
        ...
        for name in os.listdir(self.profiles_dir):
            ...
            if os.path.exists(os.path.join(profile_path, "Preferences")) or os.path.exists(
            ...
            if is_valid:
                # Get creation time
                created = datetime.datetime.fromtimestamp(os.path.getctime(profile_path))

                profiles.append({"name": name, "path": profile_path, "created": created, "type": is_valid})
    ...
    async def interactive_manager(self, crawl_callback=None):
        ...
        while True:
            ...
            if choice == "1":
                # Create new profile
                self.console.print(
                    "[green]Enter a name for the new profile (or press Enter for auto-generated name): [/green]", end=""
                )
                name = input()
                ...
                if config_json:
                    ...
                    if "webSocketDebuggerUrl" in config_json:
                        self.logger.info(
                            ...
                            tag="CDP",
                            params={"webSocketDebuggerUrl": config_json["webSocketDebuggerUrl"]},
    ...
    async def launch_builtin_browser(
        self, browser_type: str = "chromium", debugging_port: int = 9222, headless: bool = True
    ) -> Optional[str]:
        """
    ...
    async def get_builtin_browser_status(self) -> Dict[str, Any]:
        ...
        return {"running": True, "cdp_url": browser_info.get("cdp_url"), "info": browser_info}
