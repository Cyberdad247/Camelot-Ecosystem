...
def parse_srcset(s: str) -> List[Dict]:
    ...
    for part in s.split(","):
        ...
        if len(parts) >= 1:
            url = parts[0]
            width = parts[1].rstrip("w").split(".")[0] if len(parts) > 1 and parts[1].endswith("w") else None
...
class WebScrapingStrategy(ContentScrapingStrategy):
    ...
    def process_element(self, url, element: PageElement, **kwargs) -> Dict[str, Any]:
        ...
        internal_links_dict = {}
        external_links_dict = {}
        self._process_element(url, element, media, internal_links_dict, external_links_dict, **kwargs)
        return {
            "media": media,
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                raise Exception(f"Error processing links: {str(e)}")  # noqa
            ...
            try:
                ...
                raise "Error processing images"  # noqa
    ...
    def _scrap(
        ...
        **kwargs,
        ...
        if body is None:
            raise Exception(
                "'<body>' tag is not found in fetched html. Consider adding wait_for=\"css:body\" to wait for body tag to be loaded into DOM."
            )
        ...
        media["images"] = [
            img
            for result in (self.process_image(img, url, i, len(imgs), **kwargs) for i, img in enumerate(imgs))
