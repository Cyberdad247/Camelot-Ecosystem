...
async def create_source_chat_session(
    request: CreateSourceChatSessionRequest, source_id: str = Path(..., description="Source ID")
):
...
async def get_source_chat_sessions(source_id: str = Path(..., description="Source ID")):
    ...
    try:
        ...
        relations = await repo_query(
            "SELECT in FROM refers_to WHERE out = $source_id", {"source_id": ensure_record_id(full_source_id)}
        )
...
async def get_source_chat_session(
    source_id: str = Path(..., description="Source ID"), session_id: str = Path(..., description="Session ID")
):
    ...
    try:
        ...
        thread_state = source_chat_graph.get_state(config=RunnableConfig(configurable={"thread_id": session_id}))
...
async def stream_source_chat_response(
    ...
    try:
        # Get current state
        current_state = source_chat_graph.get_state(config=RunnableConfig(configurable={"thread_id": session_id}))
        ...
        result = source_chat_graph.invoke(
            ...
            config=RunnableConfig(configurable={"thread_id": session_id, "model_id": model_override}),
        )
        ...
        if "context_indicators" in result:
            context_event = {"type": "context_indicators", "data": result["context_indicators"]}
    ...
    try:
        ...
        return StreamingResponse(
            stream_source_chat_response(
                session_id=session_id, source_id=full_source_id, message=request.message, model_override=model_override
            ),
