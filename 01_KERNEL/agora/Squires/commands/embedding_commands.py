...
async def embed_single_item_command(
    ...
    try:
        logger.info(f"Starting embedding for {input_data.item_type}: {input_data.item_id}")
        ...
        if input_data.item_type == "source":
            ...
            raise ValueError(f"Invalid item_type: {input_data.item_type}. Must be 'source', 'note', or 'insight'")

        processing_time = time.time() - start_time
        logger.info(f"Successfully embedded {input_data.item_type} {input_data.item_id} in {processing_time:.2f}s")
        ...
        for idx, source_id in enumerate(items["sources"], 1):
            try:
                ...
                if idx % 10 == 0 or idx == len(items["sources"]):
                    logger.info(f"  Progress: {idx}/{len(items['sources'])} sources processed")
        ...
        for idx, insight_id in enumerate(items["insights"], 1):
            try:
                ...
                if idx % 10 == 0 or idx == len(items["insights"]):
                    logger.info(f"  Progress: {idx}/{len(items['insights'])} insights processed")
