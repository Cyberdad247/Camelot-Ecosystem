...
async def execute_command(request: CommandExecutionRequest):
    ...
    try:
        ...
        return CommandJobResponse(
            job_id=job_id, status="submitted", message=f"Command '{request.command}' submitted successfully"
        )

    except Exception as e:
        logger.error(f"Error submitting command: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to submit command: {str(e)}") from e
...
async def list_command_jobs(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list command jobs: {str(e)}") from e
...
async def cancel_command_job(job_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to cancel command job: {str(e)}") from e
...
async def debug_registry():
    ...
    try:
        ...
        for item in all_items:
            try:
                command_items.append(
                    {"app_id": item.app_id, "name": item.name, "full_id": f"{item.app_id}.{item.name}"}
                )
        ...
        return {"total_commands": len(all_items), "commands_by_app": commands_dict, "command_items": command_items}

    except Exception as e:
        logger.error(f"Error debugging registry: {str(e)}")
        return {"error": str(e), "total_commands": 0, "commands_by_app": {}, "command_items": []}
