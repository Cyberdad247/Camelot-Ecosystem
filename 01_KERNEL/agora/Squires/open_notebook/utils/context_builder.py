...
class ContextBuilder:
    ...
    def __init__(self, **kwargs):
        ...
        if context_config_arg is None:
            self.context_config = ContextConfig(
                include_insights=self.include_insights, include_notes=self.include_notes, max_tokens=self.max_tokens
            )
        else:
    ...
    async def _add_source_context(self, source_id: str, inclusion_level: str = "insights") -> None:
        ...
        try:
            ...
            item = ContextItem(id=source.id or "", type="source", content=source_context, priority=priority)
            self.add_item(item)
...
async def build_notebook_context(
    notebook_id: str, context_config: Optional[ContextConfig] = None, max_tokens: Optional[int] = None
) -> Dict[str, Any]:
    """
