...
const useAnalyticsData = (dateRange, platforms) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [sentiments, userSent, categorySent, trending, sentimentTime, influential, engagement] =
          ...
          {categoryChartData.map((cat, idx) => (
            <div key={idx} className="flex h-8 mb-1 last:mb-0">
              <div className="w-24 pr-2 flex items-center justify-end">
                <span className="text-right text-gray-300 text-sm truncate">{cat.name}</span>
              </div>
                        ...
                        sentiment === 'positive'
                          ...
                          : sentiment === 'negative'
                            ...
                            : sentiment === 'critical'
                              ? '249, 115, 22'
                              : '107, 114, 128'
                      }, ${Math.min((cat[sentiment] / (cat.total || 1)) * 0.8 + 0.1, 0.9)})`,
                    }}
                  >
