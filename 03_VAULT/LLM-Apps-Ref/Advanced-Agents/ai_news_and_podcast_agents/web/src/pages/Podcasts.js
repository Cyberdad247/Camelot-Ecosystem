...
const Podcasts = () => {
  ...
  useEffect(() => {
    ...
    fetchPodcasts();
  }, [page, perPage, selectedLanguage, selectedTtsEngine, searchQuery, hasAudio, dateFrom, dateTo]);
    ...
    <div className="max-w-6xl mx-auto">
            ...
            <div className="relative w-8 h-8">
              ...
              >
                <path
                  ...
                  strokeWidth="1.5"
                />
                <path d="M12 18V23" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                <path d="M8 23H16" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                <path
          ...
          >
            ...
            <div className="flex h-full items-center">
              <div className="flex items-center justify-center h-full">
                <div className="mt-5">
                  <label htmlFor="showOnlyActive" className="ml-0 block text-sm text-gray-300">
                    Has Audio
                  </label>
                  <ToggleSwitch
                    isActive={hasAudio}
                  ...
                  <div className="h-32 flex items-center justify-center bg-gradient-to-r from-gray-900 to-gray-800 border-b border-gray-700 relative">
                    <div className="h-14 w-14 bg-gradient-to-b from-gray-700 to-gray-800 rounded-full flex items-center justify-center border border-gray-600 shadow-lg">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className={`h-8 w-8 ${
                          hoveredItem === podcast.identifier ? 'text-emerald-400' : 'text-gray-400'
                ...
                <div className="p-4">
                  <h2
                    className={`text-md font-medium mb-1 transition-colors duration-200 line-clamp-2 ${
                      hoveredItem === podcast.identifier ? 'text-emerald-300' : 'text-gray-200'
                    }`}
                  >
      ...
      {!loading && !error && podcasts.length > 0 && (
        <div className="mt-6 flex items-center justify-between bg-gradient-to-r from-gray-800 to-gray-900 p-3 rounded-sm border-t border-gray-700">
          <div className="flex items-center text-xs text-gray-400">
            Showing <span className="font-medium text-gray-300 px-1">{podcasts.length}</span> of{' '}
            <span className="font-medium text-gray-300 px-1">{totalItems}</span> results
          </div>
          <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-end">
            <nav className="inline-flex -space-x-px rounded-sm shadow-sm" aria-label="Pagination">
              <button
                onClick={() => setPage(1)}
                disabled={!hasPrev}
