        ...
        <div className="grid grid-cols-1 gap-4">
          {articles.map((article, index) => (
            ...
            >
              ...
              <Link to={`/articles/${article.id}`} className="block p-5 relative overflow-hidden">
                {article.featured && (
                  <div
                  ...
                  <div className="flex-1">
                    ...
                    <h3
                      className={`text-lg font-medium mb-2 ${
                        hoveredCard === article.id ? 'text-emerald-300' : 'text-gray-200'
                      } transition-colors duration-200`}
                    >
                      {article.title}
                    </h3>
                    {article.summary && (
                      <p className="text-sm text-gray-400 line-clamp-2 mt-1">{article.summary}</p>
                    )}
                    <div className="mt-3">
                      <span
                        className={`text-xs font-medium text-gray-300 border border-gray-600 px-3 py-1 rounded-sm hover:bg-gray-700 inline-block ${
                          hoveredCard === article.id ? 'border-emerald-400 text-emerald-300' : ''
                        } transition-colors duration-200`}
                      >
                        Read more
                      </span>
      ...
      {!loading && !error && articles.length > 0 && (
        <div className="mt-6 flex items-center justify-between bg-gradient-to-r from-gray-800 to-gray-900 p-3 rounded-sm border-t border-gray-700">
          <div className="flex items-center text-xs text-gray-400">
            Showing <span className="font-medium text-gray-300 px-1">{articles.length}</span> of{' '}
