...
const Sources = () => {
  ...
  const handleDeleteSource = async (sourceId) => {
    if (
      window.confirm('Are you sure you want to delete this source? This action cannot be undone.')
    ) {
      try {
  ...
  const handleToggleSourceStatus = async (sourceId, currentStatus) => {
    ...
    try {
      ...
      setSources(
        sources.map((source) =>
          source.id === sourceId ? { ...source, is_active: updatedSource.data.is_active } : source,
        ),
      );
    } catch (err) {
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-3">
                        <ToggleSwitch
                          ...
                          onChange={() => handleToggleSourceStatus(source.id, source.is_active)}
                        />
                        <span
                          className={`text-xs ${
      ...
      {!loading && !error && sources.length > 0 && (
        <div className="mt-6 flex items-center justify-between bg-gradient-to-r from-gray-800 to-gray-900 p-3 rounded-sm border-t border-gray-700">
          <div className="flex items-center text-xs text-gray-400">
            Showing <span className="font-medium text-gray-300 px-1">{sources.length}</span> of{' '}
