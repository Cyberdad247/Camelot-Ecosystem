...
const PodcastSession = () => {
  ...
  const startPollingForCompletion = (taskId = null) => {
    ...
    pollTimerRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (statusResponse.data.session_id && statusResponse.data.session_id !== currentSessionId) {
          console.error(
  ...
  const bannerUrlFull = useMemo(() => {
    return podcastInfo.bannerUrl ? `${api.API_BASE_URL}/podcast_img/${podcastInfo.bannerUrl}` : '';
        ...
        <header className="sticky top-0 z-10 bg-[#0A0E14] border-b border-gray-700 shadow-md">
          <div className="h-16 px-4 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <PodcastAssetsToggle isVisible={isPreviewVisible} onClick={handleTogglePreview} />
            </div>
          </div>
          <div className="sm:hidden px-4 py-1.5 border-t border-gray-700 flex items-center justify-center bg-[#121824]/50">
            <div className="text-xs text-gray-400 flex items-center">
              ...
              ></span>
              <span>
                {isProcessing ? `Processing ${processingType || ''}...` : `Stage: ${currentStage}`}
              </span>
            </div>
          </div>
        </header>
              ...
              <FinalPresentation
                ...
                recordingUrl={
                  webSearchRecording && sessionId
                    ? `${api.API_BASE_URL}/stream-recording/${sessionId}/${webSearchRecording
                        .split('/')
                        .pop()}`
                    : ''
                }
            ...
            <>
              <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {sessionState.show_sources_for_selection && sessionState.search_results && (
                    <SourceSelection
                  ...
                  {sessionState.show_script_for_confirmation && sessionState.generated_script && (
                    <ScriptConfirmation
                  ...
                  {sessionState.show_banner_for_confirmation && sessionState.banner_url && (
                    <BannerConfirmation
                    ...
                    <div className="relative flex items-center">
                      <input
                        ...
                        className={`w-full bg-[#121824] text-white border ${
                          isProcessing || loading ? 'border-gray-600' : 'border-gray-700'
