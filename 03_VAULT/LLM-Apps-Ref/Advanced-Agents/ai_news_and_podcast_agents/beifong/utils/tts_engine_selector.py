...
def register_default_engines():
    ...
    def piper_generator(script, output_path, language_code, silence_duration, voice_map):
        from agora.swarms.piper_tts import create_podcast as piper_create_podcast

        if voice_map is None:
            voice_map = {1: "tasha", 2: "merlin"}
        return piper_create_podcast(
            script=script,
            output_path=output_path,
            silence_duration=silence_duration,
            voice_map=voice_map,
    ...
    register_tts_engine("piper", piper_generator)
