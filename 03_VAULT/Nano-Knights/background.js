...
function unwrapPreciseContract(contract) {
  ...
  if (
    ...
    contract.payload.result.service === 'precise_mode'
  ) {
...
async function buildOptimizedContext() {
  ...
  try {
    ...
    console.log(
      `[SENTINEL] Context Compressed: ${memoryNodes.length} -> ${compressed.nodes.length} nodes`,
    );
  } catch (e) {
...
async function runPreciseLane(tabId, unit, contract, optimizedContext) {
  ...
  for (let stepIndex = 1; stepIndex <= maxSteps; stepIndex++) {
    ...
    const nextPlan = await planPreciseNextAction(
      unit,
      contract,
      observation,
      stepIndex,
...
async function deployPreciseMission(request) {
  const contract = unwrapPreciseContract(
    request.contract ||
      (await chrome.storage.local.get('preciseMissionContract')).preciseMissionContract,
  );
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'GET_PRECISE_CONTRACT') {
    (async () => {
      const data = await chrome.storage.local.get([
        'preciseMissionContract',
        'preciseMissionRecord',
        'preciseMissionLastLedger',
      ]);
      sendResponse({
  ...
  if (request.action === 'GET_PROFILES') {
    chrome.storage.local.get(null, (data) => {
      ...
      if (profiles.length === 0)
        profiles.push({ id: 'default', name: 'Default (Chromium)', type: 'DESKTOP' });
  ...
  if (request.action === 'AUTH_LOGIN') {
    (async () => {
      const res = await self.AuthManager.login(
        request.email,
        request.pass,
        request.code,
        request.ghToken,
