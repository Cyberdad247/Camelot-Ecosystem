...
//
// Voice-First Cartridge — authenticated same-origin PCM frame ingress.
//
// Re-homed from the purged `02_FORGE/apps/pwa-cockpit`
// `src/app/api/voice/frames/route.ts` (deleted in 944e4532). Trust boundary is
// unchanged: the browser calls only this same-origin route, and this route may
// forward only to a loopback OmniVoice endpoint. Raw PCM is forwarded and
...
} from '@/lib/voice/voice-operator-auth';
...
export async function POST(request: NextRequest) {
  if (!isAuthorized(request)) {
    return NextResponse.json(
      { accepted: false, message: 'Operator pairing required.' },
      { status: 401 }
    );
  }
  if (isCrossSiteRequest(request)) {
    return NextResponse.json(
      { accepted: false, message: 'Cross-site voice frames are blocked.' },
      { status: 403 }
    );
  }
  ...
  if (!developmentBypass && !operatorCapabilities(request).includes('voice.use')) {
    return NextResponse.json(
      { accepted: false, message: 'The operator session lacks voice.use.' },
      { status: 403 }
    );
  }
  if (
    !request.headers.get('content-type')?.toLowerCase().startsWith('application/octet-stream')
  ) {
    return NextResponse.json(
      { accepted: false, message: 'Content-Type must be application/octet-stream.' },
      { status: 415 }
    );
  }
  ...
  if (
    ...
    return NextResponse.json(
      { accepted: false, message: 'Invalid voice frame metadata.' },
      { status: 400 }
    );
  }
  ...
  if (declaredLength > MAX_FRAME_BYTES) {
    return NextResponse.json(
      { accepted: false, message: 'Voice frame exceeds 3,200 bytes.' },
      { status: 413 }
    );
  }
  ...
  if (body.byteLength < 2 || body.byteLength > MAX_FRAME_BYTES || body.byteLength % 2 !== 0) {
    return NextResponse.json(
      { accepted: false, message: 'Voice frame must contain bounded Int16 PCM.' },
      { status: 413 }
    );
  }
  ...
  if (!endpoint) {
    return NextResponse.json(
      { accepted: false, message: 'OmniVoice loopback endpoint is invalid.' },
      { status: 503 }
    );
  }
  try {
    ...
    return NextResponse.json(
      {
        accepted: false,
        message: error instanceof Error ? error.message : 'OmniVoice unavailable.',
      },
