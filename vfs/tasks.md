...
|---|---|---|---|---|---|---|
| **`T-A1`** | **Port Native Rust `camelot_edge`**<br/>Port `kinetic_edge/camelot_edge/` from `feat/android-edge-supervisor` into workspace. | `SIR_FORGE` | `beaver_01` | *None (Root)* | `READY` | `kinetic_edge/camelot_edge/src/` |
| **`T-A2`** | **Register Workspace Members**<br/>Add `kinetic_edge/camelot_edge` to root `Cargo.toml`. | `SIR_CODEX` | `mantis_01` | `T-A1` | `READY` | `Cargo.toml` |
| **`T-A3`** | **Port Python Edge Bus & Contracts**<br/>Port `control_plane/dispatch/edge_bus.py`, `edge_protocol.py`, and `vps_mobile_mesh_bridge.py`. | `SIR_HERMES` | `falcon_01` | `T-A1` | `READY` | `control_plane/dispatch/edge_*.py` |
| **`T-A4`** | **Deploy Termux Mobile Scripts**<br/>Validate `start-camelot-edge` for Motorola Moto G & Samsung Galaxy S26 Ultra. | `SIR_LANCELOT` | `wolf_01` | `T-A3` | `READY` | `kinetic_edge/camelot_edge/termux/` |
| **`T-A5`** | **Verify Rust Edge Contract Suites**<br/>Execute `cargo check` and `cargo test -p camelot_edge`. | `ANYA_Ω` | `owl_01` | `T-A2`, `T-A4` | `READY` | Green cargo test receipt |
...
| **`T-B3`** | **Package VPS Operator Console Cartridge**<br/>Extract Go/HTMX console from `cartridge/vps-hub-cartridge-v1` into `cartridges/vps-operator-console/`. | `SIR_BORIS` | `beaver_01` | *None (Root)* | `READY` | `cartridges/vps-operator-console/` |
| **`T-B4`** | **Inscribe Cartridge System Instructions**<br/>Generate and verify `system_instruction.md` for new console cartridge. | `SIR_HELIOS` | `chameleon_01`| `T-B3` | `READY` | `vfs/cartridges/.../tether.json` |
| **`T-B5`** | **Register in WorldTree Manifest**<br/>Update `vfs/worldtree_manifest.json` and `cloudbrain_connector.py`. | `SIR_HELIOS` | `elephant_01` | `T-B4` | `READY` | `vfs/worldtree_manifest.json` |
...
| **`T-C2`** | **Parallel Scaffolding & Code Splicing**<br/>Run Beaver workers to generate glue code and contract interfaces. | `SIR_FORGE` | `beaver_01` | `T-C1` | `READY` | Spliced contract stubs |
| **`T-C3`** | **AST Auto-Repair & Lint Defense**<br/>Run Octopus worker to intercept syntax and typing errors in parallel branches. | `SIR_DEBUG` | `octopus_01` | `T-C2` | `READY` | Clean AST trees |
...
| **`T-C5`** | **Bio-Swarm Health & Telemetry Pulse**<br/>Transmit 60-second micro-loop status pulse under CamouflageCipher. | `LADY_APIS` | `falcon_01` | `T-C3` | `READY` | Living swarm telemetry |
...
| **`T-D4`** | **Graphiti & MemCastle Telemetry Inscription**<br/>Record temporal fact triplets and Tier-2 KNN embeddings under `SIR_HELIOS`. | `SIR_HELIOS` | `elephant_01` | `T-D3` | `COMPLETED` | Graphiti Facts #19, #20 |
| **`T-D5`** | **Z3 Proof Verification**<br/>Execute Z3 formal solver verifying authority invalidation and capability leases. | `SIR_GIDEON` | `owl_01` | `T-D4` | `READY` | Formal Z3 proof certificate |
...
| Task ID | Task Description | Assigned Knight | Bio-Fauna | Dependencies | Status | Artifact Output |
|---|---|---|---|---|---|---|
| **`T-O1`** | **Run Pre-Commit Parity Gates**<br/>Execute all 6 canonical check scripts (`scripts/check_*.py`). | `ANYA_Ω` | `owl_01` | `T-A5`, `T-B5`, `T-D5` | `READY` | Pre-commit pass receipt |
| **`T-O2`** | **Run Canonical Test Suite**<br/>Execute pytest testpaths (`tests/` and `configs/tests/`) with 100% green pass. | `SIR_CODEX` | `beaver_01` | `T-O1` | `READY` | 55+ passing tests |
| **`T-O3`** | **Synchronize Provenance Ledger Mirrors**<br/>Sync root, vault, docs, and configs mirrors with byte-identical SHA-256. | `SIR_HELIOS` | `elephant_01` | `T-O2` | `COMPLETED` | Synchronized LEDGER |
