...
// AuthMiddleware validates API key (Phase 8 Hardening)
func AuthMiddleware() fiber.Handler {
	return func(c fiber.Ctx) error {
		// In a production scenario, load this from config or vault
		const KineticToken = "camelot-kinetic-v300-auth-token"

		apiKey := c.Get("X-API-Key")
		authHeader := c.Get("Authorization")

		if apiKey != KineticToken && authHeader != "Bearer "+KineticToken {
			log.Warn().
				Str("ip", c.IP()).
				Str("path", c.Path()).
			...
			return c.Status(401).JSON(fiber.Map{
				"error": "Unauthorized: Kinetic Token Required",
				"code":  "unauthorized",
			})
		}
