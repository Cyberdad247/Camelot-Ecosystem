...
import (
	...
	"github.com/Denis-Chistyakov/Saltare/internal/router/soul"
...
// Knight Routing Handlers

// Route handles POST /api/v1/route
// Routes an intent to a knight engine
func (h *Handler) Route(c fiber.Ctx) error {
	...
	decision := h.soulRouter.Route(req.Intent, req.Velocity, req.Magnitude, req.Privacy)

	resp := types.RouteResponse{
		KnightID:        decision.KnightID,
		Engine:          decision.Engine,
		Weight:          decision.Weight,
		Score:           decision.Score,
		Reason:          decision.Reason,
		PrivacyOverride: decision.PrivacyOverride,
		Tensor: map[string]interface{}{
			"velocity":    decision.Tensor.Velocity,
			"magnitude":   decision.Tensor.Magnitude,
			"privacy":     decision.Tensor.Privacy,
			"environment": decision.Tensor.Environment,
		},
	}

	log.Info().
		Str("intent", req.Intent).
		Str("knight", decision.KnightID).
		Float64("score", decision.Score).
		Msg("Intent routed to knight")
